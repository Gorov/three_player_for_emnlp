...
#         self.lambda_continuity = 0.2
        ...
        self.ngram = 4
        self.with_lm = False
        self.lambda_acc_gap = 1.2 #1.5 #float(sys.argv[1])
        self.batch_size_ngram_eval = 5
        self.lr=0.001
        self.working_dir = 'beer_single_working_dir'
#         self.model_prefix = 'beer_single_balanced.%s.highlight%.2f.cont%.2f'%(self.game_mode, 
        self.model_prefix = 'beer_single_balanced.intros.%s.highlight%.2f.cont%.2f.lambda_acc_gap%.2f'%(self.game_mode, 
                                                                             self.count_tokens, 
                                                                             self.count_pieces,
                                                                             self.lambda_acc_gap)
...
reload(sys.modules['rationale_3players_sentence_classification_models'])
reload(sys.modules['util_functions'])
from rationale_3players_sentence_classification_models import HardIntrospection3PlayerClassificationModel
from util_functions import copy_classifier_module, evaluate_introspection_model_glue_for_acl

classification_model = HardIntrospection3PlayerClassificationModel(embeddings, args)
...
# best_dev_acc = evaluate_rationale_model(classification_model, beer_data, args, dev_accs, dev_anti_accs, dev_cls_accs, best_dev_acc, eval_accs, print_train_flag=False)
# print dev_accs
# print dev_anti_accs
# print dev_cls_accs
# print eval_accs
...
#     classification_model.highlight_percentage = args.highlight_percentage + (1.0 - args.highlight_percentage) * (1 - (float(i) / num_iteration))
    ...
    tuples = classification_model.train_one_step(batch_x_, batch_y_, z_baseline, batch_m_)
    losses, predict, anti_predict, cls_predict, z, z_rewards, consistency_loss, continuity_loss, sparsity_loss = tuples
    ...
    if (i+1) % display_iteration == 0:
        print 'sparsity lambda: %.4f'%(classification_model.lambda_sparsity)
        print 'highlight percentage: %.4f'%(classification_model.highlight_percentage)
        print 'supervised_loss %.4f, ssparsity_loss %.4f, continuity_loss %.4f'%(losses['e_loss'][0], torch.mean(sparsity_loss).cpu().data[0], torch.mean(continuity_loss).cpu().data[0])
        if args.with_lm:
    ...
    if (i+1) % test_iteration == 0:
#         new_best_dev_acc = evaluate_introspection_model_glue_for_acl(classification_model, beer_data, args, dev_accs, dev_anti_accs, dev_cls_accs, best_dev_acc, print_train_flag=False)
...
# #             torch.save(classification_model.state_dict(), snapshot_path)
        ...
        new_best_test_acc = evaluate_introspection_model_glue_for_acl(classification_model, beer_data, args, test_accs, test_anti_accs, test_cls_accs, best_test_acc, print_train_flag=False, eval_test=True)
